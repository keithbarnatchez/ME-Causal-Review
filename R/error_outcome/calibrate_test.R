...
out <- mclapply(1:n.sim, function(i, ...) {
  ...
  eta_out <- 0.25*x[,1] - 0.75*x[,2] + 0.75*x[,3] - 0.25*x[,4] +
    0.25*(a - 10) - 0.25*x[,1]*(a - 10)^2
...
rownames(out_cp) <- c("Naive","Bias Corrected")
...
legend("topleft", legend=c("True ERF","Naive","Bias Corrected"),
